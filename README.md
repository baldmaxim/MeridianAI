...
- 🖥 **Онлайн-встречи** — захват звука вкладки/экрана (Zoom / Teams / Meet) вместе с микрофоном: слышно обе стороны, а сторона реплики («мы» / «оппонент») определяется по источнику звука
...
```mermaid
flowchart LR
  Mic["🎙 Микрофон · наша сторона"] --> Mix["Микс · AudioWorklet"]
  Tab["🖥 Звук встречи · сторона оппонента"] --> Mix
  Mix -->|"PCM 16 kHz · WS binary"| WS["FastAPI WS · /ws/meeting"]
  Mix -.->|"уровни источников · сторона реплики"| WS
...
В онлайн-встрече голос второй стороны идёт в наушники, а не в микрофон (и подавляется эхоподавлением),
поэтому браузер дополнительно захватывает звук вкладки/экрана и подмешивает его в тот же mono-поток.
Разделение источников даёт сторону реплики почти достоверно: нашего голоса в захвате встречи нет.
Очный сценарий (только микрофон) работает как раньше — захват экрана опционален.
