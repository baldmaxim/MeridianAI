...
def build_manual_cards_prompt(role_name: str, meeting_context_block: str, recent_dialog: str,
    ...
    parts += [
        ...
        recent_dialog or "(нет)",
        "",
        f"ЗАДАЧА: верни 1–{max_cards} карточек — каждая отвечает на ПОСЛЕДНЮЮ реплику собеседника.",
        "Типы выбирай по сути: если есть риск — risk/fixation; если давят по цене/срокам/"
        "ответственности — counter/trade_concession; если не хватает фактов — ask/clarify. "
        "Не добавляй карточку ради количества или разнообразия типов: лучше две точные, чем пять общих.",
        "Фрагменты документов подобраны поиском и могут быть не по теме: опирайся только на пункты, "
        "которые прямо касаются предмета последней реплики, остальные не упоминай.",
