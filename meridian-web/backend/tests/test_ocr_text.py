...
def test_page_as_broken_json_blocks_keeps_text():
    """Кавычки внутри текста не экранированы — JSON невалиден, текст всё равно нужен."""
    raw = ('[{"role": "Text", "text": "3.2.3. Работы не по РД со штампом "В производство работ" переделываются."}, '
           '{"role": "Page-Footer", "text": "18"}]')
    assert ocr_markup_to_text(raw) == '3.2.3. Работы не по РД со штампом "В производство работ" переделываются.'


def test_broken_list_block_keeps_items():
    """Реальный блок со стр. 29: ключ list и сбитые скобки массива."""
    raw = ('[{"role": "Text", "text": "6.11. Если Работы не прошли Испытания:"}, '
           '{"role": "List-Group", "list": ["- либо дается указание о дополнительных Испытаниях;"], '
           '"- либо направляется требование о выполнении заново Работ."}, '
           '{"role": "Page-Footer", "text": "29"}]')
    assert ocr_markup_to_text(raw) == ("6.11. Если Работы не прошли Испытания:\n"
                                       "- либо дается указание о дополнительных Испытаниях;\n"
                                       "- либо направляется требование о выполнении заново Работ.")
