...
Дата: 2026-06-16 · Стенд: `app.example.com`
...
| Параметр | Значение |
|---|---|
| Стенд | https://app.example.com (environment=production, v0.10.0) |
| Backend image | `ghcr.io/your-org/meridian-api:5779e7bc…` (api + worker), `meridian-frontend:5779e7bc…` |
...
> Деплой — образы из GHCR (immutable SHA-tag), без сборки на VDS. Соседние сервисы на хосте
> (Keycloak/nginx и др.) не затронуты. Секреты в отчёте не приводятся.
