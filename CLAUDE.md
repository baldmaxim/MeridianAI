...
Работает с ElevenLabs/Deepgram/Speechmatics (транскрипция) и OpenRouter (LLM). Поддерживает загрузку PDF документов (договоры, ВОР, сметы) как контекст для подсказок.
...
| Образы из registry, без build на проде (§19) | **Сборка локально, на прод-сервере — только копирование** готовых артефактов (`docker save`/`load` или registry pull). Build на прод-сервере запрещён | Соответствует §19. host-nginx на :443, портал публикует edge на 127.0.0.1:8080. Переходно (нет CI/registry): образы собираются на dev-машине и переносятся на прод-сервер |
...
- **Деплой**: только portal-scoped — деплой Meridian не трогает nginx, Keycloak и другие сервисы на хосте. Никаких `git pull`/`npm install`/build на production VPS — только готовые образы из registry с immutable tags.
- **Сборка**: СНАЧАЛА собрать локально (dev/CI), ПОТОМ выложить. На прод-сервере — только `docker load` готовых образов + `up`. Никакого `docker build`/`npm`/`pip`/vite на прод-сервере (ограниченная RAM + соседние сервисы — сборка душит box). Поток: `docker build` → `docker save | ssh <прод-сервер> docker load` → `ssh <прод-сервер>` migrate + `up -d`.
...
- Корпоративный Keycloak: `auth.example.com`, realm `your-realm`. Meridian — OIDC confidential client.
...
> Рабочий путь — CI/GHCR: push в `main` → `.github/workflows/deploy.yml` собирает образы → на прод-сервере `deploy-ghcr.sh`.
...
- ✅ **3-PROD** — Боевой передеплой на прод-сервер (2026-06-11): `/opt/portals/app`, build-на-сервере, edge на 127.0.0.1:8080, свежая БД + бэкап старой, §7 роли. Соседние сервисы не тронуты. Live: app.example.com health 200/200, auth/rate-limit/headers ✓
...
- ✅ **5** — Файлы: presigned S3 для batch-аудио (§15, upload-session/confirm, soft-delete, физ. удаление через job) · **вживую на проде** (S3-совместимое хранилище, tenant-проект; server round-trip + браузерный CORS-preflight проверены). Graceful fallback на multipart без S3. Документы — пока multipart (под-шаг позже)
...
- ✅ **CI/GHCR** — GitHub Actions собирает образы → GHCR (public) → прод-сервер `docker pull` (deploy-ghcr.sh). Локальный Docker сломан (cred-helper), сборка-на-сервере = OOM. Это рабочий путь деплоя

## Known Issues

- ElevenLabs, Deepgram, Speechmatics и OpenRouter — платные API
