...
Работает с ElevenLabs/Deepgram/Gemini/Speechmatics (транскрипция) и OpenRouter (LLM). Поддерживает загрузку PDF документов (договоры, ВОР, сметы) как контекст для подсказок.
