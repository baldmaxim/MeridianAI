...
| Образы из registry, без build на проде (§19) | **Сборка на vds** (`docker-compose.fvds.yml`) | Registry/CI пока нет; vds уже собирал ai-helper. Registry — отдельный этап. host-nginx на :443, портал публикует edge на 127.0.0.1:8080 |
...
- ✅ **3** — Деплой: deploy/portal compose + infra-nginx + build.sh/deploy.sh с health gates · compose/скрипты валидны; живой деплой на VPS ждёт инфру
